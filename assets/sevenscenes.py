...
class Sevenscenes(torch.utils.data.Dataset):
    ...
    def __init__(self, root, seed=None, ttype='test.txt', sequence_length=2, sequence_gap=20, transform=None,
                 height=240, width=320):
        np.random.seed(seed)
        random.seed(seed)
        self.root = Path(root)

        self.scene_list_path = '7scenes'
        ...
        self.test_seqs_list = ['chess/seq-03',
                               ...
                               'stairs/seq-01',
                               'stairs/seq-04']
        scenes = [self.root / seq for seq in self.test_seqs_list]
        self.ttype = ttype
    ...
    def crawl_folders(self, sequence_length, sequence_gap):
        ...
        for scene in self.scenes:
            ...
            isc += 1
            frames = glob.glob(os.path.join(scene, "*color*"))
            frames = [int(os.path.splitext(os.path.basename(frame))[0]) for frame in frames]
            frames = sorted(frames)
            poses = [np.loadtxt(os.path.join(scene, "frame-%0d.pose.txt" % frame)) for frame in frames]

            intrinsics = np.asarray([[577.87, 0, 319.5],
                                     [0, 577.87, 239.5],
            ...
            for i in range(len(frames)):
                ...
                img = os.path.join(scene, "frame-%0d.color.png" % i)
                depth = os.path.join(scene, "frame-%06d.depth.png" % i)
                ...
                sample = {'intrinsics': intrinsics, 'tgt': img, 'tgt_depth': depth, 'ref_depths': [], 'ref_imgs': [],
                          'ref_poses': [], 'path': []}
                ...
                if i < sequence_gap:
                    ...
                    if sequence_length % 2 == 1:
                        ...
                        if (i >= demi_length * sequence_gap) and (i < len(frames) - demi_length * sequence_gap):
                            shifts = list(range(i - (demi_length) * sequence_gap, i + (demi_length) * sequence_gap + 1,
                                                sequence_gap))
                        elif i < demi_length * sequence_gap:

                            diff_demi = (demi_length - i // sequence_gap)
                            shifts = list(range(i - (demi_length - diff_demi) * sequence_gap,
                                                i + (demi_length + diff_demi) * sequence_gap + 1, sequence_gap))
                        ...
                        if (i >= demi_length * sequence_gap) and (i < len(frames) - demi_length * sequence_gap):
                            shifts = list(
                                range(i - demi_length * sequence_gap, i + (demi_length - 1) * sequence_gap + 1,
                                      sequence_gap))
                        elif i < demi_length * sequence_gap:
                            diff_demi = (demi_length - i // sequence_gap)
                            shifts = list(range(i - (demi_length - diff_demi) * sequence_gap,
                                                i + (demi_length + diff_demi - 1) * sequence_gap + 1, sequence_gap))
                        elif i >= len(frames) - demi_length * sequence_gap:

                            diff_demi = (demi_length - (len(frames) - i - 1) // sequence_gap)
                            shifts = list(range(i - (demi_length + diff_demi - 1) * sequence_gap,
                                                i + (demi_length - diff_demi) * sequence_gap + 1, sequence_gap))
                        else:
                            print('Error')
                        shifts.remove(i)
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            imgs, depths, intrinsics = self.transform([tgt_img] + ref_imgs, [tgt_depth] + ref_depths,
                                                      np.copy(sample['intrinsics']), self.height, self.width)
            tgt_img = imgs[0]
            tgt_depth = depths[0]
            ref_imgs = imgs[1:]
            ref_depths = depths[1:]
        ...
        return tgt_img, ref_imgs, ref_poses, intrinsics, tgt_depth, ref_depths, sample['tgt']
